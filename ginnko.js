...
// 组合上面两种分组函数
const combineGroup = (arr) => {
  let result = {};
  const temArr = groupByLength(arr);
  for (let key in temArr) {
    result = {
      ...result,
      ...groupByLetter(temArr[key])
    }
  }
  return result;
};
...
const checkWords = (arr) => {
  const baseWord = arr[0];
  const len = baseWord.length;
  const baseSet = [];
  // 保存其余不满足baseSet的单词集合
  const other = [];
  for (let i = 0; i < len; i++) {
    const before = baseWord.substr(0, i);
    const after = baseWord.substr(i);
    baseSet.push(after + before);
  }
  const arrLen = arr.length;
  const tem = [];
  for (let i = 0; i < arrLen; i++) {
    if (baseSet.indexOf(arr[i]) < 0) {
      other.push(arr[i]);
    } else {
      tem.push(arr[i]);
    }
...
// 计算
const cal = (arr) => {
  const result = combineGroup(arr);
  for (let key in result) {
    checkWords(result[key]);
  }
};

cal(data);
console.log(finalResult);
console.log('总计:', finalResult.length);
