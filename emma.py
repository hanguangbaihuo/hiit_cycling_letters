...
def step_two(data):
    ...
    compare_item = data[0]
    origin_list = [compare_item,]

    for item in data[1:]:
        compare_letter = compare_item[0]
        index = item.index(compare_letter)
        temp = item[index:] + item[0:index]
        if temp == compare_item:
